...
import Cookies from 'js-cookie'
  export default {
    methods: {
        ...
        getAllparks(){
          var url_temp='http://localhost:8000/allparks'
          axios.get(url_temp).then((data) => {
            for(let i=0;i<data.data.parks.length;i++){
              this.slides.push(data.data.parks[i].id);
              this.srcs.push(require('../../assets/img/'+data.data.parks[i].picture))
              this.title.push(data.data.parks[i].parkname)
              this.brief_intro.push(data.data.parks[i].detail)
            }
