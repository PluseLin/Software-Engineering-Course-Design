    ...
    methods: {
        ...
        getAllparks(){
          ...
          axios.get(url_temp).then((data) => {
            for(each in data.parks){
              this.slides.push(each.id);
              this.srcs.push(require('../../assets/img/'+each.picture))
              this.title.push(each.parkname)
              this.brief_intro.push(each.detail)
            }
          }) 
        }
    },
